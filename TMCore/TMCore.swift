...
public class TMCore {
    ...
    public func distance(locationA: CLLocation, locationB: CLLocation) -> Double {
        let distance = locationA.distance(from: locationB)
        let roundedDistance = Double(round((distance/1000)*10)/10)
...
// MARK: - 轉換 Converter
    ...
    // 時間字串轉時間字串
    public func converter(_ dateString: String,inputFormatterStr: String, outputFormatterStr: String) -> String {
        let date = converter(dateString, formatterStr: inputFormatterStr)
        return converter(date, formatterStr: outputFormatterStr)
    }
...
//MARK: - 時間
    ...
    public func addSeconds(date: Date, seconds: TimeInterval) -> Date {
...
// MARK: - NavigationController
    public func setAppNavigationBarDefault(vc: UIViewController, color: UIColor) {
        vc.navigationController?.navigationBar.barTintColor = color
        vc.navigationController?.navigationBar.tintColor = WHITE
        vc.navigationController?.navigationBar.titleTextAttributes = [NSForegroundColorAttributeName: WHITE]
    }
    
    public func setAppNavigationBarDark(vc: UIViewController, color: UIColor) {
        self.setAppNavigationBarDefault(vc: vc, color: color)
...
// MARK: - Alert
    ...
    public func showResponseErrorAlert(vc: UIViewController, msg: String?) {
        ...
        let alert = UIAlertController(title: "error".localized, message: (msg == nil ? "伺服器發生異常" : msg!), preferredStyle: .alert);
        let cancelAction = UIAlertAction.init(title: "確定", style: .cancel, handler: {(action) in })
        alert.addAction(cancelAction)
        vc.present(alert, animated: true, completion: nil)
    }
...
// MARK: - 版本
    // 取得Version與Build字串
    public func version(includingEnvironment: Bool) -> (versionNum: String, buildNum: String, environment: String) {
        var version = ""
        var build = ""
        var environment = ""
